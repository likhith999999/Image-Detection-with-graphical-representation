...
def plot_bar_chart(image, ax, title):
    if len(image.shape) == 3:
        colors = ('b', 'g', 'r')
        for i, col in enumerate(colors):
            hist = cv2.calcHist([image], [i], None, [256], [0, 256])
            ax.bar(range(256), hist[:, 0], color=col, alpha=0.5)
    else:
        hist = cv2.calcHist([image], [0], None, [256], [0, 256])
        ax.bar(range(256), hist[:, 0], color='k', alpha=0.5)
    ax.set_xlim([0, 256])
...
def showFilters():
    ...
    fig, axes = plt.subplots(len(nameOfFilters), len(nameOfImages) * 3, figsize=(45, 20))
    fig.tight_layout(pad=3.0)
    
    for i, filterSet in enumerate(filters):
        for j, img in enumerate(filterSet):
            ax_image = axes[i, j * 3]
            ax_hist = axes[i, j * 3 + 1]
            ax_bar = axes[i, j * 3 + 2]

            ax_image.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
            ax_image.axis('off')
            ax_image.set_title(f'{nameOfImages[j]} - {nameOfFilters[i]}')

            plot_histogram(img, ax_hist, f'{nameOfImages[j]} - {nameOfFilters[i]}')
            plot_bar_chart(img, ax_bar, f'{nameOfImages[j]} - {nameOfFilters[i]}')
